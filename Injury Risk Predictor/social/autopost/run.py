...
def main() -> int:
    ...
    print(f"[run] {args.format} | {league} | {args.matchday} | API={config.API_BASE}")
    candidates = fetch.fetch_candidates(args.competition)
    ...
    if not payload.get("should_post"):
        print("[run] should_post=false (no candidates). Nothing rendered or sent.")
        return 0
